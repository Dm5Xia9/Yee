...
namespace Yee.Admin.Services
{
    public class RouteState
    {
        ...
        public Type GetCurrentStyleType(NavigationManager navigation)
        {
            var dbState = _factory.Create(typeof(PageDbContext));
            if (dbState.IsWorked == false)
                return null;
            ...
            var page = db.YeeRoutes
                .Include(p => p.Page).FirstOrDefault(p => p.LocalPath == uri.LocalPath);


            return page?.Page?.StyleLink?.ToType();
        }
    }
}
